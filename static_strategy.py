...
def find_static(stock_alias, rf=0, date_frozen=10):
    """
    ...
    optimal_leverge = average / (deviation * deviation)
...
for i in range(len(tick_dict)):
    tmp_list = find_static(i, rf=0, date_frozen=trading_period)
...
def put_in_pocket(date):
    ...
    for i in range(len(tick_dict)):
        ...
        if date not in date_sort_by_date_list[i] or date + trading_period not in date_sort_by_date_list[i]:
            ...
            if optimal_leverge_list[i] <= 0:
                pass
            else:
                stock_list.append(i)
                optimal_lev_single.append(optimal_leverge_list[i])
